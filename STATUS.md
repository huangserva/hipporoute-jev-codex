...
- 完成 107 项离线单元测试，覆盖委托提取、子线程独立决策、同线程并发只决策一次、GC、真实压缩 fixture、Jev 重试/熔断、SSE 异常生命周期、原始流捕获、断开后排空、长父线程和调参基准驱动。
...
- 完成 T3 Luna effort 基准：54 个机械任务有效会话全部通过；`low` 相对默认 `max` 将请求数中位数降低 41.7%、墙钟降低 44.6%、费用中位数降低 40.3%，默认值仍保持 `max`。
...
1. 扩大带人工质量标签的子任务样本，重点扩充 Luna 置信度 0.35–0.5 的边界案例；T3 支持机械任务使用 `luna@low`，但在修改默认值前仍需覆盖更高风险任务。
